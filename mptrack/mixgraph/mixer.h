...
inline void resample_and_mix_no_interpolation(sample_t &left, sample_t &right, modplug::mixer::MODCHANNEL *source, buf_t in_sample, int smp_pos, int fixedpt_pos) {
...
template <typename buf_t>
inline void resample_and_mix_windowed_sinc(sample_t &left, sample_t &right, modplug::mixer::MODCHANNEL *source, buf_t in_sample, int smp_pos, int fixedpt_pos) {
    int pos = smp_pos + (fixedpt_pos >> 16);
    ...
    const double *fir_table =
        ((source->nInc > 0x13000 || source->nInc < -0x13000) ?
...
inline void hurfa(sample_t *left, sample_t *right, modplug::mixer::MODCHANNEL *source, size_t length/*, uint32_t flags*/) {
    const buf_t *in_sample = reinterpret_cast<buf_t *>(source->pCurrentSample);
    int pos = source->nPos;
