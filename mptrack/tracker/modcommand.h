...
struct modcommand_t {
	...
	void SetValueVolCol(const uint16_t val) {volcmd = static_cast<uint8_t>(val >> 8); vol = static_cast<uint8_t>(val & 0xFF);}

	uint16_t GetValueEffectCol() const {return GetValueEffectCol(command, param);}
	static uint16_t GetValueEffectCol(uint8_t command, uint8_t param) {return (command << 8) + param;}
