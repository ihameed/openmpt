...
//--------------------------------------------------
{
    ...
    if ((gdwSysInfo & SYSMIX_SSE) && (gdwSoundSetup & SNDMIX_ENABLEMMX))
    {
        ...
        modplug::mixer::float_to_mono_mix(MixFloatBuffer, pbuffer, nCount*2, m_pConfig->getFloatToInt());
    ...
    {	
        ...
        {
            if ((gEQ[br].bEnable) && (gEQ[br].Gain != 1.0f)) EQFilter(&gEQ[br], MixFloatBuffer+modplug::mixgraph::MIX_BUFFER_SIZE, nCount);
        }

        modplug::mixer::float_to_stereo_mix(MixFloatBuffer, MixFloatBuffer+modplug::mixgraph::MIX_BUFFER_SIZE, pbuffer, nCount, m_pConfig->getFloatToInt());
    }
}
