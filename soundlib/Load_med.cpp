...
//---------------------------------------------------------------------
{
    ...
    {
        modsample_t *pSmp = &Samples[iSHdr+1];
        pSmp->loop_start = BigEndianW(pmsh->sample[iSHdr].rep) << 1;
        pSmp->loop_end = pSmp->loop_start + (BigEndianW(pmsh->sample[iSHdr].replen) << 1);
        pSmp->default_volume = (pmsh->sample[iSHdr].svol << 2);
        ...
        if (pSmp->loop_end) pSmp->flags |= CHN_LOOP;
    }
    // Common Flags
    ...
    } else
    {
        ...
        for (UINT iSection=0; iSection<nSections; iSection++)
        {
            ...
            if ((pseq) && (pseq < dwMemLength - sizeof(MMD2PLAYSEQ)))
            {
                ...
                if (this->song_name.empty()) assign_without_padding(this->song_name, pmps->name, 31);
    ...
    for (UINT iSmp=0; iSmp<m_nSamples; iSmp++) if (pdwTable[iSmp])
    {
        ...
        } else
        {
            if (stype & 0x10)
            {
                Samples[iSmp+1].flags |= CHN_16BIT;
                len /= 2;
