...
public:
    ...
    virtual BOOL OnInitDialog();
    virtual void Init(modplug::tracker::modevent_t&)=0;
...
public:
    ...
    void Init(modplug::tracker::modevent_t &m) { m_nNote = m.note; m_nInstr = m.instr; }
    void UpdateDialog();
...
public:
    ...
    void Init(modplug::tracker::modevent_t &m) { m_nVolCmd = m.volcmd; m_nVolume = m.vol; m_bIsParamControl = (m.IsPcNote()) ? true : false;}
    void UpdateDialog();
    void UpdateRanges();
...
protected:
    ...
    modplug::tracker::modevent_t* m_pModcommand;

public:
    CPageEditEffect(CModDoc *pModDoc, CEditCommand *parent):CPageEditCommand(pModDoc, parent, IDD_PAGEEDITEFFECT) {}
    // -> CODE#0010
    // -> DESC="add extended parameter mechanism to pattern effects"
    void Init(modplug::tracker::modevent_t &m) { m_nCommand = m.command; m_nParam = m.param; m_pModcommand = &m; m_bIsParamControl = (m.IsPcNote()) ? true : false; m_nPlugin = m.instr; m_nPluginParam = modplug::tracker::modevent_t::GetValueVolCol(m.volcmd, m.vol);}
...
protected:
    ...
    modplug::tracker::modevent_t m_Command;
    bool m_bModified;
