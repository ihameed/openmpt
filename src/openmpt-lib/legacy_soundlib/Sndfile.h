...
public:    // for Editing
    ...
    // Modsequences. Order[x] returns an index of a pattern located at order x of the current sequence.
    modplug::tracker::deprecated_modsequence_list_t Order;

    // Sample Headers
    std::array<modplug::tracker::modsample_t, MAX_SAMPLES> Samples;

    // Instrument Headers
    modplug::tracker::modinstrument_t *Instruments[MAX_INSTRUMENTS];

    // Currently only used to get default values for extented properties.
    modplug::tracker::modinstrument_t m_defaultInstrument;

    // Song and sample names
    CHAR m_szNames[MAX_SAMPLES][MAX_SAMPLENAME];
    std::string song_name;

    // Midi macro config table
    MODMIDICFG m_MidiCfg;

    // Mix plugins
    SNDMIXPLUGIN m_MixPlugins[MAX_MIXPLUGINS];

    // ADPCM compression LUT
...
public:
    ...
    std::unique_ptr<modplug::tracker::eval_state_ty> eval_state;
